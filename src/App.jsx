...
function App() {
  ...
  const [aiPreferences, setAiPreferences] = useState({ ai_mode: 'agendar', default_mobile_view: 'entry' })
    ...
    if (user) {
      const fetchAiPrefs = async () => {
        const { data, error } = await supabase
          .from('profiles')
          .select('ai_mode, default_mobile_view')
          .eq('id', user.id)
          .single()
        ...
        if (data) {
          ...
          if (isMobile && isFirstVisit) {
            setCurrentPage(data.default_mobile_view === 'entry' ? 'entry' : data.default_mobile_view)
  ...
  const renderPage = () => {
    ...
    switch (currentPage) {
      ...
      case 'chat':
        return <AIChatView onBack={() => setCurrentPage('dashboard')} aiMode={aiPreferences.ai_mode} />
      case 'pending_actions':
        return <PendingActions onBack={() => setCurrentPage('dashboard')} />
      case 'entry':
        return <MobileEntryScreen onNavigate={(page) => setCurrentPage(page)} />
      default:
        return <Dashboard setPage={setCurrentPage} setSelectedAccount={setSelectedAccount} accountId={activeAccountId} />
    }
  }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="flex min-h-[calc(100vh-57px)] lg:min-h-screen w-full">
        <main className="flex-1 overflow-x-hidden p-4 sm:p-6 lg:p-8 order-last basis-0">
          {currentPage !== 'chat' && (

...
import { LayoutDashboard, Wallet, Target, PieChart, Users, Settings, Moon, Sun, CircleHelp, Download, Sparkles, Clock } from 'lucide-react'
...
export default function Sidebar({ isOpen, onClose, currentPage, setPage, setSelectedAccount, selectedAccount, signOut, accounts, isMobile, user }) {
  ...
  const items = [
    ...
    { name: 'Asistente IA', page: 'chat', icon: Sparkles },
    { name: 'Acciones Pendientes', page: 'pending_actions', icon: Clock },
  ...
  return (
    <>
      {isMobile && isOpen && <div className="fixed inset-0 z-[998] bg-black/50 backdrop-blur-[1px]" onClick={onClose}></div>}
      <aside className={`h-dvh w-[17rem] bg-card border-r border-border flex flex-col overflow-hidden ${isMobile ? 'fixed inset-y-0 left-0 z-[999] transform transition-transform duration-200 ease-out' : 'sticky top-0'} ${isMobile && isOpen ? 'translate-x-0' : isMobile && !isOpen ? '-translate-x-full' : ''}`}>
